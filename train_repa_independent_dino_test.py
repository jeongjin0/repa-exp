...
def main(args):    
    ...
    train_dataset = CIFAR10WithEmbeddings(
        cifar_base, 
        embedding_path="./embeddings/dinov2_cifar10_train.pt"
    )
    ...
    for epoch in range(args.epochs):
        model.train()
        for x1, y, target_embeddings in train_dataloader:
            x1 = x1.to(device)  # (B, 3, 32, 32)
            y = y.to(device)
            target_embeddings = torch.zeros(x1.size(0), 384, device=device)  # Dummy target embeddings
