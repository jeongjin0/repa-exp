...
from models.unet_1d import DenoisingUNet1D
...
def preprocess_for_resnet(x):
    ...
    x: [-1, 1] range의 이미지
    """
    # CIFAR-10 normalization (이미 학습시 사용한 normalization)
...
def load_resnet_encoder(checkpoint_path, device):
    ...
    resnet.eval()
    requires_grad(resnet, False)
    
    print(f"ResNet loaded from epoch {checkpoint['epoch']}")
    return resnet
...
class CIFAR10WithEmbeddings(torch.utils.data.Dataset):
    def __init__(self, cifar_dataset, embedding_path):
        self.cifar_dataset = cifar_dataset
        
        # 미리 계산된 embedding 로드
        print(f"Loading precomputed embeddings from {embedding_path}...")
        checkpoint = torch.load(embedding_path)
        self.embeddings = checkpoint['embeddings']  # [N, 384]
...
def main(args):    
    ...
    if args.use_resnet:
        resnet_encoder = load_resnet_encoder(args.resnet_checkpoint, device)
    else:
        resnet_encoder = None
    ...
    model = DiTZeroflowintegrated_independent_multitoken(original_model, noise_dim=384, output_noise_dim=384).to(device)
    ...
    loss_fn = IndependentFlowMatchingWithProjectionLossdino(
        encoders=encoders,
        accelerator=accelerator,
    )
    ...
    cifar_base = datasets.CIFAR10(root="./data/cifar10", train=True, download=True,
        transform=transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                ]),)
    train_dataset = cifar_base
    ...
    for epoch in range(args.epochs):
        ...
        for x1, y in train_dataloader:
            ...
            if args.use_resnet:
                with torch.no_grad():
                    x1_preprocessed = preprocess_for_resnet(x1)
                    _, target_embeddings = resnet_encoder(x1_preprocessed, return_embedding=True)
            else:
            ...
            if args.use_encoder and args.proj_coeff > 0:
                with torch.no_grad():
                    ...
                    z = encoders[0].forward_features(x1_preprocessed)
            ...
            with accelerator.accumulate(model):
                ...
                flow_loss, proj_loss, loss_y = loss_fn(model, x1, y, target_embeddings, device, model_kwargs, zs=zs)
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--use-resnet", action="store_true",
                       help="Use ResNet for target embeddings instead of precomputed")
    parser.add_argument("--resnet-checkpoint", type=str, default="/workspace/repa-exp/checkpoint_epoch_50.pt",
